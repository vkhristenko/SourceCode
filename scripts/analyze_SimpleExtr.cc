...
Double_t cut_toUse;
Double_t minQ_Extr_1TS = 12;
Double_t maxQ_Extr_1TS = 16;
Double_t minQ_Extr_2TS = 16;
Double_t maxQ_Extr_2TS = 26;
Double_t minQ_Extr_ToUse = minQ_Extr_1TS;
Double_t maxQ_Extr_ToUse = maxQ_Extr_1TS;
...
void analyze_SimpleExtr(string inFileName, string outFileName, 
		string outTxtFileName, string mapFileName, string gcfFile, 
		string eMapFileName, string gainMapFileName, int numTS)
{
	analyze(inFileName, outFileName, outTxtFileName,
			mapFileName, gcfFile, 
			eMapFileName, gainMapFileName, numTS);
}
		...
		string eMapFileName, string gainMapFileName, int numTS)
{
	...
	if (numTS==2)
	{
		minQ_Extr_ToUse = minQ_Extr_2TS;
		maxQ_Extr_ToUse = maxQ_Extr_2TS;
	}
	...
	pSPEvsETA_CORR[1] = new TProfile("SignalvsETA_D2_CORR", 
			"SignalvsETA_D2_CORR", 
			13, 29, 42);
	pSPEvsPHI_Avg[0] = new TProfile("SignalvsPHI_D1_Avg", 
			"SignalvsPHI_D1_Avg",
			72, 0, 72);
	pSPEvsPHI_Avg[1] = new TProfile("SignalvsPHI_D2_Avg", 
			"SignalvsPHI_D2_Avg",
			72, 0, 72);
	...
	gDirectory->mkdir("SIG-BACK");
	gDirectory->mkdir("SIG-BACK-EXT");
	gDirectory->mkdir("SIG_NORM");
	gDirectory->mkdir("BACK_NORM");
	out->cd("Histos/2D");
...
//						continue;
				...
				//
				//	Scale SIG histo up to BG histo
				//	Do SIG - BG and obtain extrapolated histo
				//
				out->cd("Histos/1D/SIG_NORM");
				scale(hRAW_S, hRAW_B);
//				norm(hRAW_S);
				hRAW_S->Write();				

				out->cd("Histos/1D/BACK_NORM");
//				norm(hRAW_B);
				hRAW_B->Write();
				out->cd("Histos/1D/SIG-BACK");
				sprintf(histName, "1D_S-B_0_PHI%d_ETA%d_D%d",
						2*iphi+1, ieta+29, idepth+1);
				TH1D *hRAW_SmB = new TH1D(histName, histName, NUMBINS,
						qBins_toUse);
				(*hRAW_SmB) = (*hRAW_S) - (*hRAW_B);
				hRAW_SmB->Write();

				out->cd("Histos/1D/SIG-BACK-EXT");
				sprintf(histName, "1D_SmB_Ext_0_PHI%d_ETA%d_D%d",
						2*iphi+1, ieta+29, idepth+1);
				TH1D *hRAW_SmB_Extr = new TH1D(histName, histName, NUMBINS,
						qBins_toUse);
				(*hRAW_SmB_Extr) = (*hRAW_S) - (*hRAW_B);

				//	
				//	Obtain SmB histos for Front and Back signals histos
				//
				scale(hRAW_S_F, hRAW_B);
				scale(hRAW_S_B, hRAW_B);
				TH1D hRAW_S_F_SmB = (*hRAW_S_F) - (*hRAW_B);
				TH1D hRAW_S_B_SmB = (*hRAW_S_B) - (*hRAW_B);

				//
				//	Calculate the average 
				//
				Double_t totContent = 0;
				Double_t totContent_F = 0;
				Double_t totContent_B = 0;
				Double_t numBinsSummed = 0;
				for (int iBin=0; iBin<NUMBINS; iBin++)
				{
					if (hRAW_SmB->GetBinCenter(iBin+1)<minQ_Extr_ToUse || 
							hRAW_SmB->GetBinCenter(iBin+1)>maxQ_Extr_ToUse)
						continue;
					totContent += hRAW_SmB->GetBinContent(iBin+1)*
						hRAW_SmB->GetBinWidth(iBin+1);
					totContent_F += hRAW_S_F_SmB.GetBinContent(iBin+1)*
						hRAW_S_F_SmB.GetBinWidth(iBin+1);
					totContent_B += hRAW_S_B_SmB.GetBinContent(iBin+1)*
						hRAW_S_B_SmB.GetBinWidth(iBin+1);
					numBinsSummed++;
				}
				totContent /= numBinsSummed;
				totContent_F /= numBinsSummed;
				totContent_B /= numBinsSummed;
				cout << "TotalContent: " << totContent << endl;
				for (int iBin=0; iBin<NUMBINS; iBin++)
				{
					if (hRAW_SmB_Extr->GetBinCenter(iBin+1)<=minQ_Extr_ToUse)
					{
						hRAW_SmB_Extr->SetBinContent(iBin+1, 
								totContent/hRAW_SmB_Extr->GetBinWidth(iBin+1));	
						hRAW_S_F_SmB.SetBinContent(iBin+1,
								totContent_F/hRAW_S_F_SmB.GetBinWidth(iBin+1));
						hRAW_S_B_SmB.SetBinContent(iBin+1,
								totContent_B/hRAW_S_B_SmB.GetBinWidth(iBin+1));
					}
				}
				hRAW_SmB_Extr->Write();
				...
				Double_t totNSum_S_Extr = 0;
				Double_t totQSum_S_Extr = 0;
				Double_t totQSum_S_F_Extr = 0;
				Double_t totQSum_S_B_Extr = 0;
				...
				for (int iBin=0; iBin<NUMBINS; iBin++)
				{
					...
					totNSum_S_Extr += hRAW_SmB_Extr->GetBinContent(iBin+1)*
						hRAW_SmB_Extr->GetBinWidth(iBin+1);
					if (iBin>floor(qie_mean_S))
					{
						totQSum_S_Extr += (hRAW_SmB_Extr->GetBinCenter(iBin+1) -
								qie_mean_S)*hRAW_SmB_Extr->GetBinContent(iBin+1)*
							hRAW_SmB_Extr->GetBinWidth(iBin+1);
						totQSum_S_F_Extr += (hRAW_S_F_SmB.GetBinCenter(iBin+1) -
								qie_mean_S)*hRAW_S_F_SmB.GetBinContent(iBin+1)*
							hRAW_S_F_SmB.GetBinWidth(iBin+1);
						totQSum_S_B_Extr += (hRAW_S_B_SmB.GetBinCenter(iBin+1) -
								qie_mean_S)*hRAW_S_B_SmB.GetBinContent(iBin+1)*
							hRAW_S_B_SmB.GetBinWidth(iBin+1);
				...
				Double_t speQ_S = totQSum_S/totNSum_S;
				Double_t speQ_B = totQSum_B/totNSum_B;
//				Double_t speQ_UNCORR = speQ_S - speQ_B;
				Double_t speQ_UNCORR = totQSum_S_Extr/totNSum_S;
				Double_t speQ_CORR = speQ_UNCORR/gcf;

				Double_t speQ_S_F = totQSum_S_F/totNSum_S_F;
				Double_t speQ_S_B = totQSum_S_B/totNSum_S_B;
				Double_t speQ_S_F_mBG = speQ_S_F - speQ_B;
				Double_t speQ_S_B_mBG = speQ_S_B - speQ_B;
				Double_t speQ_S_F_Extr = totQSum_S_F_Extr/totNSum_S_F;
				Double_t speQ_S_B_Extr = totQSum_S_B_Extr/totNSum_S_B;
				Double_t ratioSFSB = speQ_S_F_Extr/speQ_S_B_Extr;
...
void scale(TH1D *hS, TH1D *hB)
{
	Double_t totS = 0;
	Double_t totB = 0;
	for (int iBin=0; iBin<NUMBINS; iBin++)
	{
		totS += hS->GetBinContent(iBin+1)*hS->GetBinWidth(iBin+1);
		totB += hB->GetBinContent(iBin+1)*hB->GetBinWidth(iBin+1);
	}

	Double_t scaleFactor = totB/totS;
	Double_t newContent = 0;
	for (int iBin=0; iBin<NUMBINS; iBin++)
	{
			newContent = hS->GetBinContent(iBin+1)*hS->GetBinWidth(iBin+1)*
				scaleFactor/hS->GetBinWidth(iBin+1);
			hS->SetBinContent(iBin+1, newContent);
	}
}
...
void norm(TH1D* hist)
{
	Double_t numEvs = 0;
	for (int iBin=0; iBin<NUMBINS; iBin++)
		numEvs += hist->GetBinContent(iBin+1)*hist->GetBinWidth(iBin+1);
	numEvs /= pow(2,16);
	cout << "### Scaling factor=" << numEvs << endl;

	Double_t newContent = 0;
	Double_t newSum = 0;
	for (int iBin=0; iBin<NUMBINS; iBin++)
	{
		newContent = hist->GetBinContent(iBin+1)*hist->GetBinWidth(iBin+1)/numEvs/
			hist->GetBinWidth(iBin+1);
		hist->SetBinContent(iBin+1, newContent);
		newSum += newContent;
	}
	cout << "### New Total Sum=" << newSum << endl;
}
